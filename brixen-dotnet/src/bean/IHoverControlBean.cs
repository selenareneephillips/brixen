...
namespace Org.Brixen.Bean
{
	...
	public interface IHoverControlBean : IControlBean {
		...
		/// <summary>
		/// Gets or sets the <see cref="IWebElement.Click()"/> action workaround for forcing the mouse to focus on the 
		/// web control.
		/// <para>
		...
		/// </para>
		/// </summary>
		/// <value>If <c>true</c>, the <see cref="IWebElement.Click()"/> action workaround is enabled; <c>false</c> 
		/// otherwise.</value>
		...
		/// throwing any <c>Exceptions</c>, but the element is not really moused over or clicked. This will allow 
		/// automation of test cases which are dependent on the side effects generated by the hover action, but are not 
		/// related to testing that the hover action alone triggers the desired side effects. In such cases, it would 
		/// be prudent to manually test the hover action alone in the environment(s) where neither the 
		/// <see cref="OpenQA.Selenium.Interactions.Actions.MoveToElement(IWebElement)"/> nor the Javascript hover 
		/// workaround trigger the mouseover event and the expected side effect(s).
		/// </summary>
		...
		/// <see cref="OpenQA.Selenium.Interactions.Actions.MoveToElement(IWebElement)"/> method and the Javascript hover 
