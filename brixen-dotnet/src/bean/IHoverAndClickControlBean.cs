...
namespace Org.Brixen.Bean
{
	...
	public interface IHoverAndClickControlBean : IClickControlBean, IPolleableBean {
		...
		/// hover workaround and the <see cref="IWebElement.Click()"/> method all fail silently, that is they all run 
		/// without throwing any <c>Exceptions</c> but the element is not really moused over or clicked. This will 
		...
		/// would be prudent to manually test the hover action alone in the environment(s) 
		/// where neither the <see cref="OpenQA.Selenium.Interactions.Actions.MoveToElement(IWebElement)"/> nor the 
		...
		/// <summary>
		/// Gets or sets the <see cref="IWebElement.Click()"/> action workaround for the unhover element which is used 
		/// to focus the mouse in a safe location away from the web control.
		/// <para>
		...
		/// hover workaround fail silently, that is see 
